...
//search,在[first1, last1)中查找[first2, last2)的首次出现点
       ...
       ForwardIter2 first2, ForwardIter2 last2)
{
  ...
  while (current2 != last2)
  {
    ...
    else
    {
      ...
      {
        //失配之后,序列一迭代器向后,序列二迭代器归位
        current1 = ++first1;
        current2 = first2;
        --d1;//当d1减少到和d2相等时,依旧第一个元素不相等,则不再可能相配
      }
    }
  }
  return first1;
...
search_n(ForwardIter first, ForwardIter last, Size n, const T& value)
{
  ...
  else
  {
    ...
    while (first != last)
    {
      ...
      {
        //数量不够产生的失配,则继续向后找下一个与目标值相同的子序列的首位元素
                  ...
                  forward_iterator_tag, forward_iterator_tag)
{
  ...
  else
  {
    ...
    while (true)
    {
      ...
      //当找到最后一个子序列时候,此时result指向这个子序列的首部,
      // new_result再经过一遍search会指向last1,此时返回result即可
      if (new_result == last1)
      {
        return result;
      }
                  ...
                  bidirectional_iterator_tag, bidirectional_iterator_tag)
{
  ...
  //在区间(rlast1,reviter1(last1)]上反向寻找(rlast2,reviter2(last2)]
  //从后往前倒着寻找
  ...
  {
    //找到的话,再把迭代器纠正回来
    auto result = rresult.base();//这个时候迭代器指向相配子序列的末尾
    mystl::advance(result, -mystl::distance(first2, last2));//然后让迭代器向前移动first2-last2位
    return result;//真实的迭代器
  }
}
//上层的封装,通过迭代器的不同来决定使用哪个重载
template <class ForwardIter1, class ForwardIter2>
ForwardIter1
...
//二分法找第一个不小于value的元素,ForwardIter版本
                ...
                const T& value, forward_iterator_tag)
{
  ...
  while (len > 0)
  {
    ...
    middle = first;
    mystl::advance(middle, half);
    if (*middle < value)//在middle右边
    {
      first = middle;
      ++first;
      len = len - half - 1;
    }
    else if (value < *middle)//在middle左边
    {
      len = half;
    }
    else
...
//first作为参数输入操作unary_op,结构保存至result
...
//移出与value相同的元素, 但是不删除他们,所以array无法使用这个函数
template <class ForwardIter, class T>
